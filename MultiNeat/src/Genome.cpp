...
void Genome::BuildHyperNEATPhenotype(NeuralNetwork& net, Substrate& subst)
{
    ...
    if (subst.m_custom_connectivity.size() == 0) {
        ...
        for (unsigned int idx = 0; idx < subst.m_custom_connectivity.size(); idx++) {
            ...
            // conditions for canceling the CPPN query
            if (subst.m_custom_conn_obeys_flags && (((!subst.m_allow_input_hidden_links) && ((net.m_neurons[j].m_type == INPUT) && (net.m_neurons[i].m_type == HIDDEN)))
...
double Genome::CompatibilityDistance(Genome& a_G, Parameters& a_Parameters)
{
        ...
        // end of second genome?
        {
            ...
            if (t_g1innov == t_g2innov) {
                ...
                if (a_Parameters.WeightDiffCoeff > 0.0) {
                    ...
                    if (t_wdiff < 0)
                        t_wdiff = -t_wdiff; // make sure it is positive
        ...
        if ((m_NeuronGenes[i].Type() != INPUT) && (m_NeuronGenes[i].Type() != BIAS)) {
            ...
            if (a_G.HasNeuronID(m_NeuronGenes[i].ID())) {
                ...
                if (a_Parameters.ActivationADiffCoeff > 0.0) {
                    ...
                    if (t_A_difference < 0.0f)
                        t_A_difference = -t_A_difference;
                ...
                if (a_Parameters.ActivationBDiffCoeff > 0.0) {
                    ...
                    if (t_B_difference < 0.0f)
                        t_B_difference = -t_B_difference;
                ...
                if (a_Parameters.TimeConstantDiffCoeff > 0.0) {
                    ...
                    if (t_time_constant_difference < 0.0f)
                        t_time_constant_difference = -t_time_constant_difference;
                ...
                if (a_Parameters.BiasDiffCoeff > 0.0) {
                    ...
                    if (t_bias_difference < 0.0f)
                        t_bias_difference = -t_bias_difference;
    ...
    t_total_distance = (a_Parameters.ExcessCoeff * (t_num_excess * tnrm)) + (a_Parameters.DisjointCoeff * (t_num_disjoint * tnrm)) + (a_Parameters.WeightDiffCoeff * (t_total_weight_difference * tnml)) + (a_Parameters.ActivationADiffCoeff * (t_total_A_difference * tnmn)) + (a_Parameters.ActivationBDiffCoeff * (t_total_B_difference * tnmn)) + (a_Parameters.TimeConstantDiffCoeff * (t_total_timeconstant_difference * tnmn)) + (a_Parameters.BiasDiffCoeff * (t_total_bias_difference * tnmn)) + (a_Parameters.ActivationFunctionDiffCoeff * (t_total_num_activation_difference * tnmn));
